...
export class ID<T = string> implements UID<T> {
   ...
   */
  isEqual(id: UID<unknown>): boolean {
   ...
   */
  deepEqual(id: UID<unknown>): boolean {
   ...
   */
  static short(id?: string | number): ID<string> {
    const _id = new ID(id)
    if (typeof id === 'undefined') {
      _id.setAsNew()
    }
   ...
   */
  static create<T = string | number>(id?: T): ID<string> {
    return new ID(id)
  }
}
