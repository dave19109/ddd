import type { DEvent, EventName, Handler, Metrics, Options, PromiseHandler } from '../types'
...
export class TsEvents<T> {
  ...
  dispatchEvent(eventName: EventName, ...args: unknown[]): void | Promise<void> {
    const _event = this._events.find((evt): boolean => evt.eventName === eventName)
    if (!_event) {
      return
    }
   ...
   */
  addEvent(eventName: EventName, handler: Handler<T>, options?: Options): void {
   ...
   * @private
   */
  private validateHandler(handler: Handler<T>, eventName: EventName): void {
   ...
   * @private
   */
  private validateEventName(eventName: EventName): void {
   ...
   */
  removeEvent(eventName: EventName): void {
    this._events = this._events.filter((event): boolean => event.eventName !== eventName)
  }
  ...
  async dispatchEvents(): Promise<void> {
    ...
    const sorted = this._events.sort((a, b): number => a.options.priority - b.options.priority)
